...
let api = function Binance(options = {}) {
    ...
    const fBookTickerConvertData = (data) => {
        let {
            ...
            s: symbol, // 交易对
            b: bestBid, // 买单最优挂单价格
            B: bestBidQty, // 买单最优挂单数量
            a: bestAsk, // 卖单最优挂单价格
            A: bestAskQty, // 卖单最优挂单数量
        } = data;
        return {
            eventType,
            eventTime,
    ...
    const dBookTickerConvertData = (data) => {
        let {
            ...
            ps: pairSymbol, // 标的交易对
            b: bestBid, //最优买单价
            B: bestBidQty, //最优买单挂单量
            a: bestAsk, //最优卖单价
            A: bestAskQty, //最优卖单挂单量
            T: timestamp, // 撮合、挂单时间
            E: eventTime, // 事件时间
        } = data;
        return {
    ...
    /**
     * Converts the spot bookTicker stream data into a friendly object
     * @param {object} data - user data callback data type
     * @return {object} - user friendly data type
     */
    const bookTickerConvertData = (data) => {
        let {
            u: updateId,
            s: symbol,
            b: bestBid,
        ...
        */
        ...
        websockets: {
            ...
            bookTickers: function bookTickerStream(
                ...
                let subscription = subscribe(
                    endpoint,
                    (data) => callback(bookTickerConvertData(data)),
                    reconnect
                );
                return subscription.endpoint;
            },
